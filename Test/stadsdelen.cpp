...
{   
	...
	for (int i = 0; i < getGebouwCoordsLength(); i++) {
		...
		if (dynamic_cast<Mhuis*>(current) != nullptr) {	
			...
			for (int j = getGebouwCoord(i).getY(); j >= (getGebouwCoord(i).getY() - 1); j--) {
    ...
    for (int i = 0; i < getMstraatCoordsLength(); i++) {
		...
		if ((begin.getX() == 0) && (begin.getY() - end.getY() == 0) && (end.getY() == (yrow - 1))) {
			std::string stNaam;			
		...
		else if ((begin.getX() == 0) && (begin.getX() - end.getX() == 0) && (end.getX() == (xrow - 1))) {
			std::string stNaam;
